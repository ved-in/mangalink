	...
	() => {
		...
		function is_chapter_read(manga_id, ch_num)
		{
			return Object.keys(read_log).some(
				k =>
				k.startsWith(`${manga_id}\x00${ch_num}\x00`)
			);
		}

		function was_visited(manga_id, ch_num, site)
		{
			return !!read_log[`${manga_id}\x00${ch_num}\x00${site}`]; // Converts to boolean
		}
		...
		return { load, is_bookmarked, toggle, remove, update_total, get_all, is_chapter_read, was_visited, mark_visited };
