const App = (
	() => {
		...
		const panel_bm_btn = document.getElementById("panel_bm_btn");
		...
		async function init()
		{
				...
				Modal.init(
					{
						...
						on_visit: (manga_id, ch_num, site) => {
							...
							const bm = Bookmarks.get_all().find(b => b.id === manga_id);
							if (bm && typeof UI !== "undefined") UI.refresh_bm_item(manga_id, bm.read_count, bm.total_chapters);
						},
			...
			if (url_q && (input || input_2)) {
				if (typeof API !== 'undefined' && document.getElementById('results_list')) {
					API.get_manga(url_q).then(manga => {
						if (manga && manga.id === url_q) {
							UI.render_manga_results([manga], 'results_list', {
								is_bookmarked: id => Bookmarks.is_bookmarked(id),
								on_select: select_manga,
								on_bookmark: toggle_bookmark,
							});
							select_manga(manga, null);
						} else {
							do_search();
						}
					}).catch(() => do_search());
			...
			if (mobile_back_btn) {
				mobile_back_btn.addEventListener("click", () => {
					if (container) container.classList.remove("manga-selected");
					if (chapters_panel) chapters_panel.style.display = "none";
					document.querySelectorAll(".manga_card, .ru-card, .bm_item").forEach(c => c.classList.remove("selected"));
				});
			}
		}
		...
		async function select_manga(manga, card_el)
		{
			...
			if (container) container.classList.add("manga-selected");
			if (panel_bm_btn) {
				panel_bm_btn.classList.toggle("bookmarked", Bookmarks.is_bookmarked(manga.id));
				panel_bm_btn.onclick = () => toggle_bookmark(manga);
			}
